...
class Config:
    ...
    OUTPUT_BASE_DIR = "./output/unified_output"
...
def run_hf_inference(model, processor, image: Image.Image, prompt: str, model_name: str):
    ...
    if "Qwen" in model_name:
        ...
        # Format the prompt with the image path
        formatted_prompt = f'<img>{temp_image_path}</img>{prompt.strip()}'
    else: # Llava
    ...
    # Clean up the response by removing any image tokens
    return response.replace("<img>", "").strip()
    ...
    for idx in tqdm(range(cfg.N_SAMPLES), desc="Processing Samples"):
        try:
            ...
            if "Qwen" in cfg.MODEL_NAME:
                mcq_string = "\n".join([f"{v}. {k}" for k, v in mcq_dict.items()])
                prompt = prompt_template.format(mcq_dict=mcq_string)
            else:
