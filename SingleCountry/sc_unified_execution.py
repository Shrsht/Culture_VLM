...
from sc_gen_mcq import load_datasets, generate_q1, generate_q2, create_geo_dictionaries
from sc_image_join import FlagComposer
...
class Config:
    ...
    DATASET_TYPE = "single_country"  # "original" or "single_country"
    SINGLE_COUNTRY_IMAGE_DIR = "/Users/shreshtvenkatraman/Desktop/MING LAB/MOSAIC_1.5K_v2"
    SINGLE_COUNTRY_METADATA_PATH = "/Users/shreshtvenkatraman/Desktop/MING LAB/mosaic_metadata.csv"
...
# --- Dataset Loading ---
def load_single_country_dataset(image_dir, metadata_path):
    """
    Loads the single country image dataset, ensuring that it only processes
    images that are both listed in the metadata CSV and present in the image directory.
    """
    logger.info(f"Loading single country dataset from metadata: {metadata_path}")
    metadata = pd.read_csv(metadata_path)
    logger.info(f"Found {len(metadata)} entries in metadata file.")
    ...
    for _, row in metadata.iterrows():
        try:
            # Clean up filename from CSV
            image_filename = str(row['image']).strip()
            if not image_filename:
                logger.warning(f"Skipping row with empty image name: {row}")
                continue

            image_path = os.path.join(image_dir, image_filename)

            # Explicitly check if the file exists on the filesystem
            if not os.path.exists(image_path):
                logger.warning(f"Image file from metadata not found in directory, skipping: {image_path}")
                continue
            
            # Avoid processing duplicate entries from the CSV
            if image_path in processed_files:
                logger.warning(f"Skipping duplicate image entry in metadata: {image_filename}")
                continue

            country_name = str(row['country']).strip()
            if not country_name:
                logger.warning(f"Skipping row due to invalid country name: {row}")
                continue

            continent = cc.convert(names=country_name, to='continent')
            subregion = cc.convert(names=country_name, to='UNregion')
            ...
            logger.error(f"Skipping row due to an unexpected error: {e} - Row: {row}")
            continue
    
    logger.info(f"Successfully loaded {len(dataset)} images to be processed.")
    return dataset
...
def main():
    ...
    if cfg.DATASET_TYPE == "original":
        image_dataset, flag_dataset = load_datasets()
    elif cfg.DATASET_TYPE == "single_country":
        image_dataset = load_single_country_dataset(cfg.SINGLE_COUNTRY_IMAGE_DIR, cfg.SINGLE_COUNTRY_METADATA_PATH)
        _, flag_dataset = load_datasets() # Still need flags
    else:
    ...
    # --- Prompt Template ---
    prompt_template = """You are an expert on cultures. Examine the cultural context in the following image. From the list of flags below, select the flag that best represents the culture depicted in this image.
...
Your Answer:"""
    ...
    for idx, item in enumerate(tqdm(image_dataset, desc="Processing Samples")):
        try:
            ...
            if "gemini" in cfg.MODEL_NAME:
                raw_response = run_gemini_inference(model, composed_image, prompt)
            elif "gpt" in cfg.MODEL_NAME:
                raw_response = run_openai_inference(model, composed_image, prompt, cfg.MODEL_NAME)
            else: # Hugging Face models
                raw_response = run_hf_inference(model, processor, composed_image, prompt, cfg.MODEL_NAME)
